...
#define LOGI(...) fprintf(stdout,__VA_ARGS__);putchar('\n');
#define LOGE(...) fprintf(stdout,__VA_ARGS__);putchar('\n');
#define LOGD(...) fprintf(stdout,__VA_ARGS__);putchar('\n');
...
void ElfParser::print_symbols_full_name(char *symbol_name) {
	...
	if (read_section_header(STR_DYNSYM, &dynsym_header) < 0 ||
		read_section_header(STR_DYNSTR, &dynstr_header) < 0) {
...
int main(int argc, char *argv[]) {
	...
	switch (argc) {
	case 1:
		scanf("%s %s", so_name, symbol_name);
		break;
	case 2:
		strcpy(so_name, argv[1]);
		scanf("%s", symbol_name);
		break;
	case 3:
		strcpy(so_name, argv[1]);
		strcpy(symbol_name, argv[2]);
		break;
	}
	ElfParser parser(so_name);

	if (strcmp("all", symbol_name) == 0) {
		parser.print_symbols_full_name();
	}
	else {
		parser.print_symbols_full_name(symbol_name);
	}
